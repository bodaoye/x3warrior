...
PLUGINLIB_EXPORT_CLASS(
    debict::mecanumbot::controller::WarriorDriveController,
    controller_interface::ControllerInterface
)

using namespace debict::mecanumbot::controller;

WarriorDriveController::WarriorDriveController()
    : controller_interface::ControllerInterface()
...
controller_interface::InterfaceConfiguration WarriorDriveController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration WarriorDriveController::state_interface_configuration() const
{
...
controller_interface::return_type WarriorDriveController::init(const std::string & controller_name)
{   
    ...
    try {
        auto node = get_node();
        auto_declare("fl_wheel_joint_name", "");
        auto_declare("fl_wheel_joint_name", "");
        auto_declare("fl_wheel_joint_name", "");
        auto_declare("fl_wheel_joint_name", "");
        auto_declare("fl_wheel_joint_name", "");
        auto_declare("wheel_radius", 0.0);
        auto_declare("wheel_distance.width", 0.0);
        auto_declare("wheel_distance.length", 0.0);
...
controller_interface::return_type WarriorDriveController::update()
{
...
CallbackReturn WarriorDriveController::on_configure(const rclcpp_lifecycle::State &)
{
    RCLCPP_INFO(get_node()->get_logger(), "Configure warrior_drive_controller");
...
CallbackReturn WarriorDriveController::on_deactivate(const rclcpp_lifecycle::State &)
{
    return CallbackReturn::SUCCESS;
}
...
CallbackReturn WarriorDriveController::on_error(const rclcpp_lifecycle::State &)
{
    return CallbackReturn::SUCCESS;
}

CallbackReturn WarriorDriveController::on_shutdown(const rclcpp_lifecycle::State &)
{
    return CallbackReturn::SUCCESS;
}
